...
/*   Updated: 2023/11/28 17:46:48 by jaehyji          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	insert_ambient(t_data *data, char **word, int *flag)
{
	*flag |= 1 << FLAG_AMBIENT;
	...
	if (errno == ERANGE || data->amb.ratio < 0 || data->amb.ratio > 1)
...
void	insert_camera(t_data *data, char **word, int *flag)
{
	*flag |= 1 << FLAG_CAMERA;
	...
	if (errno == ERANGE || data->cam.fov < 0 || data->cam.fov > 180)
		ft_error(data, E_RANGE);
}
...
void	insert_light(t_data *data, char **word)
{
	...
	if (errno == ERANGE || temp->ratio > 1 || temp->ratio < 0)
		ft_error(data, E_RANGE);
